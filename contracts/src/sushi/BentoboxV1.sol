...
// File contracts/interfaces/IWFIL.sol
// License-Identifier: MIT
...
contract BentoBoxV1 is MasterContractManager, BoringBatchable {
    ...
    IERC20 private immutable wfilToken;
    ...
    constructor(IERC20 wfilToken_) public {
        wfilToken = wfilToken_;
    }
    ...
    function deposit(
        ...
        // Effects
        IERC20 token = token_ == USE_ETHEREUM ? wfilToken : token_;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            // X2: If the WFIL implementation is faulty or malicious, it will block adding ETH (but we know the WETH implementation)
            IWFIL(address(wfilToken)).deposit{value: amount}();
    ...
    function withdraw(
        ...
        // Interactions
        if (token_ == USE_ETHEREUM) {
            // X2, X3: A revert or big gas usage in the WFIL contract could block withdrawals, but WFIL is fine.
            IWFIL(address(wfilToken)).withdraw(amount);
